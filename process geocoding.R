...
dat <- read.csv(dat_path, stringsAsFactors = F)
# dat <- read.csv("/Users/christianbaehr/Desktop/SCA Geocoding/Pakistan/srilanka_master.csv",
#                 stringsAsFactors = F)
dat_new <- dat[1,]
dat_new$geometry <- NA
dat_new <- dat_new[0,]
...
# gadm2 <- st_read("/Users/christianbaehr/Desktop/SCA Geocoding/Pakistan/gadm36_LKA_2.geojson", 
#                  stringsAsFactors=F)
gadm2 <- st_read("/Users/christianbaehr/Desktop/shapefiles/gadm36_PAK_shp/gadm36_PAK_2.shp",
                 stringsAsFactors=F)
gadm2$centroids <- st_centroid(gadm2$geometry)
gadm2[, c("X", "Y")] <- st_coordinates(gadm2$centroids)
gadm2[, c("X", "Y")] <- round(data.frame(gadm2)[, c("X", "Y")], digits=2)
...
gadm1 <- st_read("/Users/christianbaehr/Desktop/shapefiles/gadm36_PAK_shp/gadm36_PAK_1.shp", stringsAsFactors=F)
gadm1$centroids <- st_centroid(gadm1$geometry)
gadm1[, c("X", "Y")] <- st_coordinates(gadm1$centroids)
gadm1[, c("X", "Y")] <- round(data.frame(gadm1)[, c("X", "Y")], digits=2)
gadm1$merge_id <- paste0(gadm1$X, ", ", gadm1$Y)
...
gadm0 <- st_read("/Users/christianbaehr/Desktop/shapefiles/gadm36_PAK_shp/gadm36_PAK_0.shp", stringsAsFactors=F)
...
out_file <- paste0("/Users/christianbaehr/Downloads/Pakistan_2000-17_geocoded_", as.character(Sys.Date()), ".geojson")
out_file_csv <- paste0("/Users/christianbaehr/Downloads/Pakistan_2000-17_geocoded_", as.character(Sys.Date()), ".csv")
